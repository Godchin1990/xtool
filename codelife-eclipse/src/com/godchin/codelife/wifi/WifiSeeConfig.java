 ...
 * 操作类
 */
public class WifiSeeConfig {

    private Context context;

    public WifiSeeConfig(Context context) {
    ...
    public ArrayList<String[]> get_list() {
        ...
        try {
            ...
            if (run_cmd("cp /data/misc/wifi/wpa_supplicant.conf " + path)) {
                ...
                Toast.makeText(context, "can_no_read_file", Toast.LENGTH_LONG).show();
                return null;
            }
        } catch (Exception e) {
    ...
    public boolean check_root() {
        try {
            try {
                ...
                Toast.makeText(context, "can_not_root_permission" + ":" + e.getMessage(), Toast.LENGTH_LONG).show();
            }
        } catch (Exception e) {
            return false;
        }
