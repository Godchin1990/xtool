...
/**
 * ReactiveNetwork is an Android library listening network connection state and
 * change of the WiFi signal strength with RxJava Observables. It can be easily
 * used with RxAndroid.
 */
public final class ReactiveNetwork {
	...
	/**
	 * enables Internet connection check When it's called
	 * WIFI_CONNECTED_HAS_INTERNET and WIFI_CONNECTED_HAS_NO_INTERNET statuses
	 * can be emitted by observeConnectivity(context) method. When it isn't
	 * called only WIFI_CONNECTED can by emitted by observeConnectivity(context)
	 * method.
	 * 
	...
	/**
	 * Observes ConnectivityStatus, which can be WIFI_CONNECTED,
	 * MOBILE_CONNECTED or OFFLINE
	 * 
	 * @param context
	 *            Context of the activity or an application
	...
	public Observable<ConnectivityStatus> observeConnectivity(
		...
		return Observable.create(
				new Observable.OnSubscribe<ConnectivityStatus>() {

					@Override
					public void call(
							final Subscriber<? super ConnectivityStatus> subscriber) {
						final BroadcastReceiver receiver = new BroadcastReceiver() {
							...
							public void onReceive(Context context, Intent intent) {
								...
								// because after going off-line, onReceive() is
								// called twice
								if (newStatus != status) {
									status = newStatus;
	...
	/**
	 * Observes WiFi Access Points. Returns fresh list of Access Points whenever
	 * WiFi signal strength changes.
	 * 
	 * @param context
	 *            Context of the activity or an application
	...
	public Observable<List<ScanResult>> observeWifiAccessPoints(
		...
		wifiManager.startScan(); // without starting scan, we may never receive
									// any scan results
		...
		return Observable
				.create(new Observable.OnSubscribe<List<ScanResult>>() {

					@Override
					public void call(
							final Subscriber<? super List<ScanResult>> subscriber) {
						final BroadcastReceiver receiver = new BroadcastReceiver() {
							@Override
							public void onReceive(Context context, Intent intent) {
								wifiManager.startScan(); // we need to start
															// scan again to get
															// fresh results
															// ASAP
		...
		return Subscriptions.create(new Action0() {
			...
			public void call() {
				if (Looper.getMainLooper() == Looper.myLooper()) {
					...
					final Scheduler.Worker inner = AndroidSchedulers
