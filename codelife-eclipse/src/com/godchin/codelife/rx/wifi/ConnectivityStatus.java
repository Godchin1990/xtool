...
public enum ConnectivityStatus {
	UNKNOWN("unknown"), WIFI_CONNECTED("connected to WiFi"), WIFI_CONNECTED_HAS_INTERNET(
			"connected to WiFi (Internet available)"), WIFI_CONNECTED_HAS_NO_INTERNET(
			"connected to WiFi (Internet not available)"), MOBILE_CONNECTED(
			"connected to mobile network"), OFFLINE("offline");
	...
	/**
	 * Creates a function, which checks if single connectivity status or many
	 * statuses are equal to current status. It can be used inside filter(...)
	 * method from RxJava
	 * 
	 * @param statuses
	 ...
	 */
	public static Func1<ConnectivityStatus, Boolean> isEqualTo(
	...
	/**
	 * Creates a function, which checks if single connectivity status or many
	 * statuses are not equal to current status. It can be used inside
	 * filter(...) method from RxJava
	 * 
	 * @param statuses
	 *            many connectivity statuses or single status
	 * @return Func1<ConnectivityStatus, Boolean> from RxJava
